...
public class ReservaDAO {
	...
	public HashSet<Hotel> getHotelesPorUbicacion(String ciudad) {
		...
		String getHoteles = (ciudad.compareTo("") != 0) ? "SELECT * FROM hotel WHERE hotel.ciudad = ?"
				: "SELECT * FROM hotel";
	...
	private Map<String, Tipo> getNumHabitaciones(int hotel_id) {
		...
		String getNumeroDeHabitaciones = "SELECT habitaciones.tipo, habitaciones.total_habitaciones "
		...
		try {
			...
			while (rsGetNumHabitaciones.next()) {
				disponibles.put(rsGetNumHabitaciones.getString("tipo"), new Tipo(rsGetNumHabitaciones.getString("tipo"),
						0, Integer.parseInt(rsGetNumHabitaciones.getString("total_habitaciones"))));
			}
			;
	...
	private double getPrecio(int hotel_id, String fecha, String tipo) {
		...
		String getPrecio = "SELECT precio.valor " + "FROM precio"
	...
	private HashSet<Tipo> getNumeroHabitacionesDisponibles(int hotel_id, String fecha_inicio, String fecha_fin) {
		...
		String getReservasPorDias = "SELECT reserva.habitacion_tipo, count(reserva.tipo) as 'num_reservas' "
				+ "FROM reserva " + "WHERE reserva.hotel_id = ? "
				+ "AND reserva.fecha_inicio <= ? AND reserva.fecha_fin >= ? " + "GROUP BY reserva.habitacion_tipo;";
			...
			while (nextDay != fecha_fin) {
				...
				while (rs.next()) {
					...
					if (disponibles.containsKey(rs.getString("habitacion_tipo"))) {
						...
						if (disponibles.get(rs.getString("habitacion_tipo")).getDisponibles() > Integer
								.parseInt(rs.getString("num_reservas"))) {

							disponibles.get(rs.getString("habitacion_tipo"))
									.addPrecio(getPrecio(hotel_id, day, rs.getString("habitacion_tipo")));
	...
	public int anadirReserva(String fecha_entrada, String fecha_salida, double importe, int hotel_id,
			Habitaciones.Tipo tipo, int cliente_id) {
