...
//return ( res != null ? res.intValue() : -1 );
	 ...
	 * 
	 * id : int // Esto se refiere a servicio nombre : String precio : int unidad :
	 * String
	 */
	 ...
	 * 
	 * id : int, ciudad : String, nota : double ocupacion : Map<Date,Double>
	 * (ocupación en porcentaje, ej: 0.25 -> tiene ocupado un cuarto de su
	 * capacidad)
	 * 
	 * habitaciones : List<Properties> (cada una a su vez tendrá: id: int nombre:
	 * String capacidad: int )
	 */
	...
	public double getNotaHotel(int hotel_id) {
		...
		String query = "SELECT AVG(nota) " + "FROM Valoracion " + "WHERE hotel_id=?;";
	...
	public double getOcupacionHotel(int hotel_id, Date fecha) {
		BigDecimal res = null;
		ScalarHandler<BigDecimal> handler = new ScalarHandler<>();
		String query = "SELECT COUNT(*)/capacidad AS ocupacion " + "FROM ("
				+ "SELECT hotel_id,SUM(num_disponibles) AS capacidad " + "FROM Hotel_Tipo_Habitacion "
				+ "WHERE hotel_id = ? " + ") AS hcap " + "JOIN Reserva r ON hcap.hotel_id = r.hotel_id "
				+ "WHERE ? BETWEEN r.fecha_entrada AND r.fecha_salida " + "GROUP BY r.hotel_id ";
	...
	/*
	 * id: int nombre: String capacidad: int
	 */
