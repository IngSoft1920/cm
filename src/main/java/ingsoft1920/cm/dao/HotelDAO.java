...
public class HotelDAO {
	...
	public void anadirHotel(String nombre, String continente, String pais, String ciudad, String direccion,
			Map<Habitaciones.Tipo, Integer> habitaciones) {
	...
	public void editarHotel(int id, String nombre, String continente, String pais, String ciudad, String direccion,
			Map<Habitaciones.Tipo, Integer> habitaciones) {
	...
	public List<Hotel> hoteles() {
		...
		try {
			...
			while (rs.next()) {
				hoteles.add(new Hotel(rs.getInt("id"), rs.getString("nombre"), rs.getString("continente"),
						rs.getString("pais"), rs.getString("ciudad"), rs.getString("direccion")));
			}
	...
	public List<Factura> facturacionHotel(int hotel_id) {
		...
		try {
			...
			while (rs.next()) {
				facturas.add(new Factura(rs.getInt("id"), rs.getInt("importe"), rs.getBoolean("pagado"),
						rs.getString("descripcion"), rs.getDate("fecha"), rs.getInt("cliente_id")));
			}
	...
	private Map<Hotel, List<Precio>> getHotelesPorUbicacion(String continente, String pais, String ciudad) {
		...
		String getHotelesPorUbicacion = "SELECT *  " + "FROM hotel "
		...
		try {
			...
			rs = stmt.executeQuery();
			while (rs.next()) {
				Hotel hotel = new Hotel(rs.getInt("id"), rs.getString("nombre"), rs.getString("continente"),
						rs.getString("pais"), rs.getString("ciudad"), rs.getString("direccion"));
	...
	private List<Precio> getPreciosPorFechas(int hotel_id, Date fecha_entrada, Date fecha_salida) {
		...
		try {
			...
			while (rs.next()) {
				Precio precio = new Precio(rs.getInt("hotel_id"),
						Habitaciones.Tipo.valueOf(rs.getString("habitaciones_tipo")), rs.getDate("fecha"),
						rs.getDouble("precio"));
				precios.add(precio);
			}
	...
	public Map<Hotel, List<Precio>> hotelesPorReserva(String continente, String pais, String ciudad, Date fecha_entrada,
			Date fecha_salida) {
