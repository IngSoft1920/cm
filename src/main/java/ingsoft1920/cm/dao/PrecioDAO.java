...
public class PrecioDAO {
	...
	public void setPrecio(int id_hotel, Tipo tipoHabitacion, Date fecha, float precio) {
		...
		try {
			st = conector.getConn()
					.prepareStatement("Add into precio(id_hotel, tipo_habitacion, fecha, valor) values(?, ?, ?, ?);");
	...
	public void addDatosPrecio(int idPeticion, float precio, int puntuacion) {
		...
		try {
			st = conector.getConn()
					.prepareStatement("Add into datos_precios(id_peticion, precio,puntuacion) " + "values(?,?,?);");
