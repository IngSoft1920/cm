...
public class EmpleadoDAO {
	...
	public int anadirEmpleado(Empleado empleado, Hotel_Empleado hotelEmpleado) {
		...
		String anadeEmpleado = "INSERT INTO Empleado " + "(nombre,apellidos,email,telefono,sueldo,profesion_id) "
				+ "VALUES (?, ?, ?, ?, ?, ?);";
		...
		try (Connection conn = conector.getConn()) {
			idGenerado = runner.insert(conn, anadeEmpleado, handler, empleado.getNombre(), empleado.getApellidos(),
					empleado.getEmail(), empleado.getTelefono(), empleado.getSueldo(), empleado.getProfesion_id());

			runner.insert(conn, anadeEmpleadoaHotel, handler, idGenerado, hotelEmpleado.getHotel_id(),
	...
	public void cambiarApellidos(Empleado empleado, String nuevosApellidos) {
		...
		try (Connection conn = conector.getConn()) {
			runner.update(conn, cambiaApellidos, handler, nuevosApellidos, empleado.getEmail());
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void editar(Empleado empleado) {
		...
		ScalarHandler<BigInteger> handler = new ScalarHandler<>();
		try (Connection conn = conector.getConn()) {
			runner.update(conn, cambiaEmail, empleado.getNombre(), empleado.getApellidos(), empleado.getEmail(),
					empleado.getTelefono(), empleado.getSueldo(), empleado.getId());
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
