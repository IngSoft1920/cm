...
public class ControladorCliente {
	...
	public String reservarForm(String continente,String pais,String ciudad,
							   Date fecha_entrada,Date fecha_salida,Model m) {

		int numDias = (int) ((fecha_salida.getTime() - fecha_entrada.getTime())/ 86_400_000);

		// TODO dao
		Map<Hotel,List<Precio>> seleccionHabitaciones
				= fake.habitacionesReserva(continente, pais, ciudad, fecha_entrada, fecha_salida);
		...
		for( Entry<Hotel,List<Precio>> entrada : seleccionHabitaciones.entrySet() ) {
			...
			double precioNormales = entrada
										.getValue()
										.stream()
										.filter(p->p.getTipo()==Tipo.normal)
										.mapToDouble(p->p.getPrecio())
										.sum()/numDias;
			preciosNocheHotel.put(Tipo.normal, Math.floor(precioNormales*100)/100);

			double precioPremiums = entrada
										.getValue()
										.stream()
										.filter(p->p.getTipo()==Tipo.premium)
										.mapToDouble(p->p.getPrecio())
										.sum()/numDias;
	...
	public String recibirReserva(@ModelAttribute("cliente") Cliente clienteSesion,
								 @ModelAttribute("fecha_entrada") Date fecha_entrada,
								 @ModelAttribute("fecha_salida") Date fecha_salida,
	...
	public String cancelarReserva(@ModelAttribute("cliente") Cliente clienteSesion,
								  Model m) {
